...
export function TimelineExplorer() {
  return (
    <div className="p-6 h-full flex flex-col">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-[#0a0a0a] flex items-center">
            <Clock className="w-6 h-6 mr-3 text-[#0a0a0a]" />
            Epoch Timeline
          </h1>
      ...
      <div className="flex-1 bg-[#141417] border border-[#e8e8e8] rounded p-6 relative overflow-hidden">
         <div className="absolute top-0 bottom-0 left-32 w-px bg-white"></div>
...
function TimelineEvent({ time, title, color }: any) {
  return (
    <div className="flex items-start group">
       <div className="w-24 text-right pr-6 text-xs font-mono text-slate-500 pt-1 group-hover:text-[#0a0a0a] transition-colors">{time}</div>
       <div className="relative flex items-center justify-center w-8">
          <div className={`w-3 h-3 rounded ${color} shadow-[0_0_10px_currentColor] z-10 ring-4 ring-[#141417]`}></div>
       </div>
